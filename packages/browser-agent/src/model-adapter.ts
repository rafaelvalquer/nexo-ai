...
export function buildBrowserToolCallingProbeRequest(
  model: string,
  think: "omitted" | boolean = "omitted",
) {
  const request: {
    model: string;
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a capability probe. You must call the provided function. Do not answer with natural language.",
      },
      {
        role: "user",
...
export class BrowserAgentPreflightError extends Error {
  constructor(
    ...
    public readonly metadata?: {
      model: string;
      source: BrowserAgentModelSource;
      httpStatus?: number;
    },
  ) {
...
export class NexoBrowserModelAdapter {
  constructor(
    ...
    private readonly modelSource: BrowserAgentModelSource = "global_setting",
  ) {}
  ...
  async preflight(signal?: AbortSignal): Promise<OllamaPreflightResult> {
    ...
    if (!response.ok)
      throw await this.classifyModelResponse(response, "/api/show");
    ...
    try {
      ...
      throw new BrowserAgentPreflightError(
        ...
        "O Ollama retornou uma resposta inválida ao verificar o modelo.",
        { cause: error },
      );
    }
    ...
    try {
      compatibilityResponse = await fetch(`${base}/api/chat`, {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify(
          buildBrowserToolCallingProbeRequest(this.modelId, false),
        ),
      ...
      throw classifyFetchError(
        error,
        "O Ollama não concluiu o preflight nativo de tool calling do Browser Agent.",
        this.modelId,
        this.modelSource,
      );
    }
    ...
    if (!compatibilityResponse.ok) {
      if (compatibilityResponse.status === 404)
        throw this.error(
          ...
          `O modelo selecionado não foi encontrado durante o teste nativo de tool calling.`,
    ...
    const message =
      compatibility && typeof compatibility === "object"
        ...
        : undefined;
    const inspection = inspectOllamaToolCalls(
      message,
      new Set([PREFLIGHT_TOOL_NAME]),
    );
    const expected = inspection.calls.find(
      (call) =>
        call.name === PREFLIGHT_TOOL_NAME &&
        call.arguments.url === PREFLIGHT_URL,
    );
    if (!expected) {
      throw new BrowserAgentPreflightError(
  ...
  private async assertModelInstalled(base: string, signal?: AbortSignal) {
    ...
    try {
      ...
      throw classifyFetchError(
        error,
        "Não foi possível consultar os modelos instalados no Ollama.",
        this.modelId,
        this.modelSource,
      );
    }
    if (!response.ok)
      throw new BrowserAgentPreflightError(
        ...
        `O endpoint /api/tags respondeu com HTTP ${response.status}.`,
        undefined,
        {
          model: this.modelId,
    ...
    try {
      ...
      throw this.error(
        "BROWSER_MODEL_INVALID_RESPONSE",
        "O endpoint /api/tags retornou JSON inválido.",
        response.status,
        error,
      );
    }
    ...
    if (!installed.has(this.modelId))
      throw this.error(
        "BROWSER_MODEL_NOT_FOUND",
        `O modelo selecionado não está instalado no Ollama.`,
        undefined,
      );
  }
  ...
  private async classifyModelResponse(response: Response, endpoint: string) {
    ...
    if (
      response.status === 404 ||
      /not found|does not exist|try pulling/i.test(detail)
    )
      return this.error(
        "BROWSER_MODEL_NOT_FOUND",
        `O modelo selecionado não foi encontrado por ${endpoint}. ${detail}`,
        response.status,
      );
    if (response.status === 400)
      return this.error(
        "BROWSER_MODEL_CONFIG_INVALID",
        `Ollama rejeitou a configuração do modelo em ${endpoint}. ${detail}`,
        response.status,
      );
    return this.error(
      "BROWSER_OLLAMA_UNAVAILABLE",
      `O endpoint ${endpoint} respondeu com HTTP ${response.status}. ${detail}`,
      response.status,
    );
  }
  ...
  private error(
    ...
    return new BrowserAgentPreflightError(
      code,
      `${message} Modelo: ${this.modelId}. Origem: ${this.modelSource}.`,
  ...
  async createModels(signal?: AbortSignal, preflight?: OllamaPreflightResult) {
    const base = this.ollamaUrl.replace(/\/$/, "");
    const supportsVision = preflight
      ? preflight.capabilities.includes("vision")
      : await this.detectVision(base, signal);
...
function classifyFetchError(
  ...
  fallback: string,
  model = "desconhecido",
  source: BrowserAgentModelSource = "global_setting",
) {
  ...
  if (value?.name === "TimeoutError")
    return new BrowserAgentPreflightError(
      ...
      `${fallback} Tempo limite excedido. Modelo: ${model}. Origem: ${source}.`,
      { cause: error },
      metadata,
    );
  if (value?.name === "AbortError")
    return new BrowserAgentPreflightError(
      "BROWSER_OLLAMA_TIMEOUT",
      `${fallback} A operação foi interrompida. Modelo: ${model}. Origem: ${source}.`,
      { cause: error },
      metadata,
    );
  return new BrowserAgentPreflightError(
    "BROWSER_OLLAMA_UNAVAILABLE",
    `${fallback} Modelo: ${model}. Origem: ${source}.`,
    { cause: error },
    metadata,
  );
}
...
async function readOllamaError(response: Response) {
  try {
    ...
    return "Resposta sem detalhe estruturado.";
  }
}
function sanitizeError(value: string) {
  const clean = value
    ...
    .trim()
    .slice(0, 240);
  return clean || "Ollama não informou detalhes.";
}
