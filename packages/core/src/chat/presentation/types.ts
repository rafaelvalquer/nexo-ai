...
const action = z.object({id:z.string(),icon:z.enum(["reply","archive","trash","open","edit","move","download","read","unread","more","preview","copy","search","list","rsvp"]),label:z.string(),mutation:z.boolean(),disabled:z.boolean().optional()});
...
const clarificationQuestion = z.object({ id:z.string(), field:z.string(), prompt:z.string(), type:z.enum(["single_choice","multi_choice","text","choice_or_text","email","textarea"]), options:z.array(clarificationOption).optional(), suggestedOptionId:z.string().optional(), selectedOptionIds:z.array(z.string()).optional(), allowCustomValue:z.boolean().optional(), customPlaceholder:z.string().optional(), required:z.boolean(), helperText:z.string().optional(), submitLabel:z.string().optional() });
const base = { id: z.string(), version: z.literal(1) };
const editableEmailApproval = z.object({ to:z.array(z.string()), subject:z.string(), bodyText:z.string() });
export const presentationSchema = z.object({ version: z.literal(1), blocks: z.array(z.discriminatedUnion("type", [
  ...
  z.object({ ...base, type: z.literal("approval"), approvalId: z.string(), title: z.string(), preview: z.string().optional(), consequence: z.string().optional(), affectedCount: z.number().optional(), expiresAt: z.string().optional(), status: z.enum(["pending", "approved", "rejected", "expired"]), editableEmail: editableEmailApproval.optional() }),
...
export function parsePresentation(value: unknown): ChatPresentation | undefined {const parsed = presentationSchema.safeParse(value);return parsed.success ? parsed.data as ChatPresentation : undefined;}
