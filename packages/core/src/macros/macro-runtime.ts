...
/** Runtime for macros, backed by the existing SQLite automations schema for compatibility. */
export class MacroRuntime {
  ...
  completeChatRun(runId:string,status:MacroRun["status"],summary?:string,error?:string):void {
  ...
  private install(automation:Macro):void{const nextRunAt=this.scheduler.nextRun(automation);this.repository.updateRunState(automation.id,{nextRunAt});this.scheduler.install(automation);this.smartTriggers.install(automation);}
  private uninstall(id:string):void{this.scheduler.uninstall(id);this.smartTriggers.uninstall(id);}

  private async run(automation:Macro,payload:Record<string,unknown>,ignoreEnabled=false):Promise<MacroRun|MacroExecutionResult|undefined>{
    ...
    this.runningAutomationIds.add(fresh.id);const runId=randomUUID(),controller=new AbortController();this.runControllers.set(fresh.id,controller);const context:MacroExecutionContext={automationId:fresh.id,runId,trigger:{type:fresh.trigger.type,data:payload},actionResults:{},startedAt:new Date().toISOString()};this.runs.start(fresh.id,fresh.trigger.type,payload,context);
    try{
  ...
  private async executeFrom(automation:Macro,context:MacroExecutionContext,startIndex:number):Promise<MacroRun|undefined>{
    try{
  ...
  private async executeWithRetry<T>(automation:Macro,actionType:string,action:()=>Promise<T>):Promise<T>{const canRetry=RETRY_SAFE_ACTION_TYPES.has(actionType),attempts=canRetry&&automation.policy.retries.enabled?Math.max(0,automation.policy.retries.count)+1:1;let lastError:unknown;for(let attempt=0;attempt<attempts;attempt++){try{return await action();}catch(error){lastError=error;if(!canRetry||!isTransient(error)||attempt===attempts-1)throw error;}}throw lastError;}
  private toViewModel(automation:Macro):MacroView{const running=this.runningAutomationIds.has(automation.id);const status=running?"running":automation.lastRunStatus==="waiting_approval"?"waiting_approval":automation.consecutiveFailures>=5?"attention":automation.enabled?"active":"paused";return{...automation,status,triggerLabel:triggerLabel(automation),actionSummary:actionSummary(automation)};}
}


function isV2Input(input:Omit<Automation,"id"|"lastRunAt">|CreateMacroInput):input is CreateMacroInput{return"trigger"in input&&"actions"in input;}
function legacyInputToV2(input:Omit<Automation,"id"|"lastRunAt">):CreateMacroInput{const trigger=input.triggerType==="cron"?{type:"schedule" as const,mode:"cron" as const,cron:input.schedule}:input.triggerType==="file-created"?{type:"file.created" as const,path:input.watchPath??""}:input.triggerType==="file-changed"?{type:"file.changed" as const,path:input.watchPath??""}:input.triggerType==="app-start"?{type:"app-start" as const}:{type:"manual" as const};return{name:input.name,prompt:input.command,enabled:input.enabled,trigger,conditions:[],conditionOperator:"AND",actions:[{id:"legacy-command",type:"nexo.command",config:{command:input.command}}],output:{type:"chat",conversationMode:"automation"},policy:DEFAULT_MACRO_POLICY};}
...
function triggerLabel(automation:Macro):string{const t=automation.trigger;if(t.type==="schedule")return t.cron?`Agendamento · ${t.cron}`:t.time?`${t.mode} · ${t.time}`:"Agendamento";if(t.type==="interval")return`A cada ${t.minutes} min`;if(t.type==="file.created")return"Arquivo criado";if(t.type==="file.changed")return"Arquivo alterado";if(t.type==="file.deleted")return"Arquivo removido";if(t.type==="email.received")return"Novo e-mail";if(t.type==="calendar.before_event")return`${t.minutesBefore} min antes do compromisso`;if(t.type==="calendar.event_started")return"Compromisso iniciado";if(t.type==="system.threshold")return`${t.metric} ${t.operator} ${t.threshold}`;if(t.type==="app-start")return"Ao iniciar o Nexo";return"Manual";}
function actionSummary(automation:Macro):string{if(!automation.actions.length)return"Nenhuma ação";const names=automation.actions.map(action=>action.type==="nexo.command"?"Comando Nexo":action.type==="notification.show"?"Notificação":action.type);return names.slice(0,2).join(" + ")+(names.length>2?` +${names.length-2}`:"");}
function fallbackPrompt(automation:Macro):string{const command=automation.actions.find(action=>action.type==="nexo.command")?.config.command;if(typeof command==="string"&&command.trim())return command.trim();return `Execute a automação "${automation.name}" e explique o resultado de forma objetiva.`;}
