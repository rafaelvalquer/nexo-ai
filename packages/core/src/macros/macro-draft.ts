...
import type { MacroStep } from "@nexo/shared";
import type { MacroActionCatalogItem } from "./macro-action-catalog.js";

export type GeneratedMacroDraft={name:string;description:string;actions:MacroStep[]};

export function normalizeMacroDraft(raw:string,description:string,name?:string,catalog:MacroActionCatalogItem[]=[]):GeneratedMacroDraft{
  let parsed:any;
  ...
  const actions:MacroStep[]=parsed.actions.map((candidate:any,index:number)=>{
