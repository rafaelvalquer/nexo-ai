...
import type { MacroTrigger, Macro } from "@nexo/shared";

export type MacroTriggerPayload = Record<string, unknown>;
export type MacroTriggerEmitter = (automation: Macro, payload: MacroTriggerPayload) => Promise<unknown> | unknown;
...
export class MacroScheduler {
  ...
  constructor(private emit: MacroTriggerEmitter) {}
  ...
  nextRun(automation: Macro, from = new Date()): string | undefined {
  ...
  private installSchedule(automation: Macro, trigger: Extract<MacroTrigger, { type: "schedule" }>): void {
  ...
  private installInterval(automation: Macro, minutes: number): void {
  ...
  private installFile(automation: Macro): void {
    const trigger = automation.trigger;
  ...
  private emitFileDebounced(automation: Macro, filePath: string, event: string, debounceMs: number): void {
...
export function scheduleToCron(trigger: Extract<MacroTrigger, { type: "schedule" }>): string | undefined {
