...
import type { MacroExecutionContext, MacroRunStatus, MacroStepRun, MacroRun } from "@nexo/shared";
import { NexoDatabase } from "../database/db.js";

export class MacroRunRepository {
  constructor(private db: NexoDatabase) { this.ensureSchema(); }

  start(automationId: string, triggerType: string, triggerPayload: Record<string, unknown>, context: MacroExecutionContext): MacroRun {
  ...
  finish(id: string, status: MacroRunStatus, details: { summary?: string; error?: string; approvalId?: string } = {}): void {
  ...
  waitForApproval(id: string, approvalId: string, context: MacroExecutionContext, nextActionIndex: number): void {
  ...
  updateContext(id: string, context: MacroExecutionContext, nextActionIndex: number): void {
    this.db.run("UPDATE automation_runs SET context_json=?,next_action_index=? WHERE id=?", [JSON.stringify(context), nextActionIndex, id]);
  }

  failedContext(id:string):{context:MacroExecutionContext;nextActionIndex:number}|undefined{
    ...
    try{return{context:JSON.parse(row.context_json) as MacroExecutionContext,nextActionIndex:Number(row.next_action_index??0)};}catch{return undefined;}
  }

  prepareResume(id:string,context:MacroExecutionContext,nextActionIndex:number):void{
  ...
  finishStep(id: string, status: MacroRunStatus, details: { summary?: string; error?: string; approvalId?: string } = {}): void {
  ...
  finishStepByApproval(approvalId: string, status: MacroRunStatus, details: { summary?: string; error?: string } = {}): void {
  ...
  list(automationId: string, limit = 50): MacroRun[] {
  ...
  get(id: string): MacroRun | undefined {
    const row = this.db.get<RunRow>("SELECT * FROM automation_runs WHERE id=?", [id]);
    return row ? this.mapRun(row, true) : undefined;
  }

  pendingByApproval(approvalId: string): { run: MacroRun; context: MacroExecutionContext; nextActionIndex: number } | undefined {
    ...
    return { run: this.mapRun(row, true), context: JSON.parse(row.context_json) as MacroExecutionContext, nextActionIndex: Number(row.next_action_index ?? 0) };
  }
...
function mapStep(row: StepRow): MacroStepRun { return { id:row.id,runId:row.run_id,ordinal:row.ordinal,actionId:row.action_id,actionType:row.action_type,status:asStatus(row.status),startedAt:row.started_at,finishedAt:row.finished_at??undefined,durationMs:row.duration_ms??undefined,summary:row.summary??undefined,error:row.error??undefined,approvalId:row.approval_id??undefined }; }
function asStatus(value:string):MacroRunStatus { return value === "queued" || value === "running" || value === "waiting_approval" || value === "success" || value === "failed" || value === "skipped" || value === "cancelled" ? value : "failed"; }
