...
export function validateIntentSemantics(intent: CanonicalIntent, userText: string): SemanticValidationResult {
  ...
  if (!derivedMissing.length && !isKnownOperation(intent.operation)) {
    return invalid(intent, "INVALID_INTENT_OPERATION", "Não há requisitos semânticos registrados para a operação.");
  }

  const missing = [...derivedMissing];
  ...
  for (const key of ["folder", "path", "source", "destination"]) {
    ...
    if (entity?.source === "inferred" && !missing.includes(key)) {
      missing.push(key);
      ambiguities.push({
        code: "untrusted_inferred_location",
        field: key,
        message: "O local não foi informado literalmente nem derivado de contexto validado.",
        critical: true
      });
    }
  }
  ...
  if (intent.operation === "create_folder" || intent.operation === "create_text_file") {
    ...
    if (name && unsafeLeafName(name)) {
      ambiguities.push({
        code: "unsafe_name",
        field: "name",
        message: "O nome deve representar apenas um arquivo ou pasta, sem caminho ou navegação relativa.",
        critical: true
      });
    }
  }
  if (intent.operation === "rename_file") {
    ...
    if (newName && unsafeLeafName(newName)) {
      ambiguities.push({
        code: "unsafe_name",
        field: "newName",
        message: "O novo nome deve ser apenas o nome final, sem caminho.",
        critical: true
      });
    }
  }
  ...
  if (intent.operation === "create_folder" && isResourceTypeAmbiguous(text)) {
    ambiguities.push({
      code: "resource_type",
      field: "name",
      message: "Não ficou claro se o recurso é arquivo ou pasta.",
      critical: true
    });
  }
  ...
  for (const key of ["path", "source", "destination", "folder"]) {
    ...
    if (!containsLiteralPath(text, value)) {
      ambiguities.push({
        code: "invented_physical_path",
        field: key,
        message: "O caminho físico não aparece literalmente no pedido do usuário.",
        critical: true
      });
    }
  }
  ...
  if (next.missing.length) {
    return {
      ...
      question: missingQuestion(intent.operation, next.missing[0])
    };
  }
  ...
  if (critical) {
    return {
      ...
      question: ambiguityQuestion(critical, next)
    };
  }
...
function isKnownOperation(operation: string) {
  return Boolean(intentOperationContracts[operation]);
}
...
function unsafeLeafName(value: string) {
  return containsTraversal(value) || /[\\/]/.test(value) || isAbsolutePortable(value);
}

function isAbsolutePortable(value: string) {
