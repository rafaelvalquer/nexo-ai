...
import { isComposedDocumentWorkflow, parseFileIntent } from "../agent/intent/file-intent.js";
...
export class LegacyIntentRouter {
  ...
  route(text: string, options:{allowedRoots?:string[]}={}): Plan | null {
    ...
    if(unsupportedSpreadsheet)return{direct:unsupportedSpreadsheet};
    const filesystemCommand=this.filesystemResolver.resolve(text,options.allowedRoots??[]);
    if(filesystemCommand){const command=filesystemCommandTool(filesystemCommand);return{tool:command.tool,input:command.input,explanation:command.explanation};}
    ...
    // A concrete filename is a local filesystem intent, even when its search
    // verb also matches generic web research language.
    // URL/page and signed-in account requests stay in the Web/Browser domain even
    // when their prose contains a filename-like token such as example.com.
    const browserRequest=isBrowserAgentTask(text)&&(Boolean(explicitWebUrl(text))||/\b(linkedin|instagram|youtube|github|infomoney|investing|valor|g1|uol)\b/i.test(text));
    const fileIntent=browserRequest?undefined:parseFileIntent(text);
