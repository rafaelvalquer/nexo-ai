...
export class CommandService {
  ...
  route(text: string, previous?: ConversationActionContextState): CommandRoute {
    ...
    if (!routed) return isLikelyConversation(text) ? { type: "chat", stream: true } : { type: "unknown" };
    if (routed.steps?.length) return routed.steps.length === 1 ? this.fromToolStep(routed.steps[0]) : { type: "unknown" };
    if (routed.uiFlow || routed.emailDraft || routed.deferredAction) return { type: "unknown" };
    if (typeof routed.direct === "string") return isLikelyConversation(text) ? { type: "chat", response: routed.direct } : { type: "unknown" };
    if (routed.directStream) return { type: "chat", stream: true };
    if (routed.tool) return this.fromToolStep({ tool: routed.tool, input: routed.input ?? {}, explanation: routed.explanation });
    return { type: "unknown" };
  }
