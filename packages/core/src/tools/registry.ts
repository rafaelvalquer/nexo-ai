...
export class ToolRegistry{
  ...
  get(name:string){return this.tools.get(name);}list(){return[...this.tools.values()].map(t=>({name:t.name,description:t.description,risk:t.risk,permissions:t.permissions}));}
  listForAgent(){return[...this.tools.values()].map(tool=>({name:tool.name,description:tool.description,risk:tool.risk,permissions:tool.permissions,parameters:zodToJsonSchema(tool.inputSchema,{$refStrategy:"none"})}));}
}
