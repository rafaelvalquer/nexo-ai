...
export function webReaderTools():ToolDefinition[]{
  return [
    {name:"web_search",description:"Pesquisa páginas públicas na web sem abrir navegador. Retorna títulos, URLs e trechos como conteúdo externo não confiável.",risk:"READ",permissions:["web.read"],domain:"web",operation:"search",mutatesState:false,agent:{category:"web",outputTrust:"untrusted_external"},inputSchema:z.object({query:z.string().trim().min(2).max(500),maxResults:z.number().int().min(1).max(10).default(5)}),async execute({query,maxResults},context){
      const url=new URL("https://html.duckduckgo.com/html/");url.searchParams.set("q",query);
      const response=await requestText(url,context?.signal,DEFAULT_TIMEOUT_MS,1_000_000);
      ...
      const results=parseSearchResults(response.body,url.toString()).slice(0,maxResults);
      return {ok:true,summary:`Pesquisa web: ${results.length} resultado(s) para “${query}”.`,data:{query,results,untrustedExternalContent:true}};
    }},
    {name:"web_fetch",description:"Baixa uma página HTTP/HTTPS pública e extrai seu texto principal sem navegador; retorna conteúdo externo não confiável.",risk:"READ",permissions:["web.read"],domain:"web",operation:"fetch",mutatesState:false,agent:{category:"web",outputTrust:"untrusted_external"},inputSchema:z.object({url:z.string().url().max(2048),maxChars:z.number().int().min(500).max(MAX_TEXT_CHARS).default(12_000)}),async execute({url,maxChars},context){
      const response=await requestText(new URL(url),context?.signal,DEFAULT_TIMEOUT_MS,MAX_HTML_BYTES);
...
export function parseSearchResults(html:string,baseUrl:string){
...
function requestOnce(url:URL,signal:AbortSignal|undefined,timeoutMs:number,maxBytes:number):Promise<{status:number;headers:http.IncomingHttpHeaders;body:string}>{
  return new Promise((resolve,reject)=>{
    ...
    const request=transport.request(url,{method:"GET",headers:{"accept":"text/html,application/xhtml+xml,text/plain;q=0.9,*/*;q=0.1","user-agent":"NexoAI-WebReader/1.0"},lookup:((hostname:string,options:any,callback:any)=>{
