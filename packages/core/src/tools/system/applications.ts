...
export function applicationTools(): ToolDefinition[] {
  return [
    {
      name:"open_url", description:"Abre uma URL no navegador padrão", domain:"browser", operation:"open_url", exposure:"PUBLIC_AGENT_TOOL", risk:"WRITE", mutationSafety:{idempotency:"none",reconciliation:"not_supported"},permissions:["system.open"],
      inputSchema:z.object({url:z.string().url()}), async execute({url}){await open(url);return {ok:true,summary:`URL aberta: ${url}`};}
    },
    {
      name:"open_path", description:"Abre arquivo ou pasta com o aplicativo padrão", domain:"filesystem", operation:"open_path", exposure:"PUBLIC_AGENT_TOOL", risk:"WRITE", mutationSafety:{idempotency:"none",reconciliation:"not_supported"},permissions:["system.open"], pathFields:["path"],
      inputSchema:z.object({path:z.string()}), async execute({path:target}){await open(target);return {ok:true,summary:`Aberto: ${target}`};}
    },
    {
      name:"open_application", description:"Abre um aplicativo permitido", domain:"system", operation:"open_application", exposure:"PUBLIC_AGENT_TOOL", risk:"WRITE", mutationSafety:{idempotency:"none",reconciliation:"not_supported"},permissions:["system.open"],
