...
export class CoreModuleManager {
  ...
  disable(id: string): Promise<void> { return this.serialize(() => this.disableInternal(id)); }

  private async disableInternal(id: string): Promise<void> {
    ...
    for (const dependent of dependents) await this.disableInternal(dependent.module.id);
  ...
  shutdown(): Promise<void> { return this.serialize(async () => {
    const enabled = [...this.entries.values()].filter(entry => entry.status === "ready").map(entry => entry.module.id);
    for (const id of enabled.reverse()) await this.disableInternal(id);
  }); }
