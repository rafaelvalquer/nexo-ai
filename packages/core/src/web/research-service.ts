...
export class WebResearchService{
  ...
  async research(input:WebResearchInput,signal?:AbortSignal):Promise<WebResearchResult>{
    ...
    if(!articles.length)this.metric?.("web.research.failed",1);
    return{query,source:source?{name:source.name,domain:source.domain,url:source.url}:undefined,articles,failedSources,partial,untrustedExternalContent:true};
  }
}
