...
function walk(value:unknown,output:ConversationEntity[],seen:Set<unknown>){if(!value||typeof value!=="object"||seen.has(value))return;seen.add(value);if(Array.isArray(value)){for(const item of value)walk(item,output,seen);return;}const row=value as Record<string,unknown>;const pageUrl=string(row.url);const id=string(row.id)||string(row.messageId)||string(row.documentId)||string(row.eventId)|| (pageUrl&&/^https?:\/\//i.test(pageUrl)?pageUrl:undefined);const path=string(row.path)||string(row.destination);if(id){const kind:ConversationEntity["kind"]=pageUrl&&id===pageUrl?"page":row.messageId||row.subject||row.receivedAt?"email":row.eventId||row.start&&row.end?"event":row.documentId||row.mimeType?"document":"file";output.push({kind,id,label:string(row.subject)||string(row.title)||string(row.name),path,ordinal:1});}else if(path)output.push({kind:"file",id:path,path,label:string(row.name),ordinal:1});for(const nested of Object.values(row))walk(nested,output,seen);}
