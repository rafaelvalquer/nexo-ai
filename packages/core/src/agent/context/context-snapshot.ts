...
export function contextSnapshotFromActionState(input:{conversationId:string;turn:number;state?:ConversationActionContextState;ledger?:ConversationEntity[];memoryCandidates?:RetrievedIntentExample[];recentMessages?:ContextSnapshot["recentMessages"];pendingClarification?:ContextSnapshot["pendingClarification"]}):ContextSnapshot{
  const entities:ResolvedContextEntity[]=[];
  for(const [kind,rows] of [["file",input.state?.files],["email",input.state?.emails],["event",input.state?.events]] as const){
    (rows??[]).forEach((row:any,index)=>entities.push({kind,id:String(row.id??row.path),label:row.name??row.subject??row.title,path:row.path,ordinal:index+1,turnAge:0,source:"previous_result",confidence:1}));
  }
  ...
  return{conversationId:input.conversationId,turn:input.turn,pendingClarification:input.pendingClarification,recentMessages:(input.recentMessages??[]).slice(-12),recentEntities:entities,lastAction:input.state?.lastTool?{tool:input.state.lastTool,operation:input.state.lastIntent,resultIds:entities.filter(x=>x.source==="previous_result").map(x=>x.id),turnAge:0}:undefined,lastIntent:input.state?.lastDomain&&input.state.lastIntent?{domain:input.state.lastDomain,operation:input.state.lastIntent,entities:{},turnAge:0}:undefined,memoryCandidates:input.memoryCandidates??[]};
}
