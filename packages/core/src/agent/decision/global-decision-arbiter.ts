...
export class GlobalDecisionArbiter{
 ...
 decide(input:{userText:string;candidates:DecisionCandidate[];domainEvidence:DomainEvidenceSnapshot;expectedDomain?:string;context?:ContextEvidence[];goalByKey?:Map<string,GoalSatisfaction>;failurePenaltyByKey?:Map<string,number>;hardVetoEnabled?:boolean}):GlobalDecisionResult{
  ...
  for(const candidate of dedupe(input.candidates)){
   ...
   const score=evaluateDecisionConfidence({domainConsistency,operationScore:candidate.confidence,entityCompleteness:1,goalScore:goal.score,contextConfidence:contextConfidence(input.context),deterministicEvidence:deterministicScore(candidate),verifiedMemory:candidate.source==="intent_memory"?candidate.confidence:0,ambiguityCount:candidate.ambiguities.length,conflictCount:0,failurePenalty:input.failurePenaltyByKey?.get(key(candidate))??0,mutatesState:candidate.mutatesState}).overall;
