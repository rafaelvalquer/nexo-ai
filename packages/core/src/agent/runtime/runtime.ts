...
export class AgentRuntime{
  ...
  getApplicationState<T=unknown>(key:string):T|undefined{const row=this.db.get<{value:string}>("SELECT value FROM application_state WHERE key=?",[key]);if(!row)return undefined;try{return JSON.parse(row.value) as T;}catch{return undefined;}}
