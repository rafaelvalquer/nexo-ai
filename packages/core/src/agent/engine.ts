...
export class AgentEngine{
  ...
  constructor(private planner:AgentPlanner,private registry:ToolRegistry,private permissions:PermissionEngine,private approvals:ApprovalService,private audit:AuditService,private connections?:ConnectionService,private runtime?:AgentRuntime,private security?:SecurityPolicyService,private metrics?:LocalMetricsService,private resources?:ResourceManager){this.toolCatalog=new CapabilityAwareToolCatalog(registry,connections);this.actionExecutor=new ActionExecutor(registry,permissions,audit,{security,metrics,resources});if(runtime)this.clarifications=new ClarificationService(new ClarificationRepository(runtime),new ClarificationResolver(permissions));}
