...
export class AgentEngine{
  ...
  async run(userText:string,hooks:AgentRunHooks={},context:LLMMessage[]=[]):Promise<AgentReply>{
    ...
    let plan:Plan;
    try{hooks.onStatus?.("Interpretando sua intenção com a IA local…");plan=await this.planner.plan(userText,context,hooks.signal,previous,this.toolCatalog.list());if(shadow)this.recordShadow(await shadow,plan,Date.now()-shadowStarted);}
  ...
  async resumeApproval(checkpointId:string,hooks:AgentRunHooks={}):Promise<AgentReply>{
    ...
    while(state.nextStep<state.steps.length){this.assertNotAborted(hooks.signal);if(state.iteration>=AGENT_LIMITS.maxIterations||state.results.length>=AGENT_LIMITS.maxToolCalls){const text="O fluxo retomado atingiu o limite seguro de etapas.";this.runtime.finish(resumed.run.id,"FAILED",text);return{text,results:state.results};}
      ...
      this.runtime.recordStep(resumed.run.id,state.nextStep,step,"RUNNING");hooks.onStatus?.(step.explanation??`Retomando ${step.tool}…`);hooks.onToolStarted?.(step.tool,step.explanation??`Retomando ${step.tool}`);const approved=this.approvals.list("all").find(item=>item.checkpointId===checkpointId&&item.status==="approved");const reply=await this.execute(step.tool,data,{...this.executionContext(hooks),approvalId:approved?.id});this.assertNotAborted(hooks.signal);hooks.onToolCompleted?.(step.tool,Boolean(reply.result?.ok));this.runtime.recordStep(resumed.run.id,state.nextStep,step,reply.result?.ok?"COMPLETED":"FAILED",reply.result,reply.result?.error);if(!reply.result?.ok){this.runtime.finish(resumed.run.id,"FAILED",reply.text);return{...reply,results:state.results};}
