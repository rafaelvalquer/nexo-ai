...
export type AgentReply={text:string;result?:ToolResult;results?:ToolResult[];approvalId?:string;reviewDraftId?:string;conversationId?:string;presentation?:ChatPresentation;responseMode?:"synthesize"|"deterministic"|"presentation";engine?:AgentEngineSource;fallbackReason?:string;toolsUsed?:string[];outcome?:GoalOutcome};
...
export class AgentEngine{
  ...
  async run(userText:string,hooks:AgentRunHooks={},context:LLMMessage[]=[]):Promise<AgentReply>{
    ...
    if(typeof (this.planner as any).recordCorrectionSignal==="function")(this.planner as any).recordCorrectionSignal(previous,userText);
    ...
    if(typeof (this.commandService as any).recordPlannerDecision==="function")(this.commandService as any).recordPlannerDecision(plan,typeof (this.planner as any).memoryDecisionCandidates==="function"?(this.planner as any).memoryDecisionCandidates():[]);
  ...
  private async executeSteps(userText:string,steps:PlanStep[],plan:Pick<Plan,"origin"|"intent"|"deferredAction"|"responseMode">,hooks:AgentRunHooks,context:LLMMessage[]):Promise<AgentReply>{
    ...
    const done:{step:PlanStep;result:ToolResult}[]=[];let previousContext=previous,deferredConsumed=false,modelFinalResponse:string|undefined,lastOutcome:GoalOutcome|undefined;
    ...
    for(let stepIndex=0;stepIndex<steps.length;stepIndex++){
      ...
      if(this.needsApproval(tool,needsMemoryApproval)){
        const meta=buildConfirmation(step,tool,data),reason=needsMemoryApproval?"A IA identificou uma memória para salvar e sua configuração exige confirmação.":meta.title??meta.consequence??step.explanation??"Ação requer aprovação";
      ...
      if(reply.result){
        hooks.onToolResult?.(tool.name,data,reply.result);done.push({step,result:reply.result});
        const priorContext=previousContext;
        lastOutcome=await this.verifyOutcome(userText,tool.name,reply.result,hooks.signal);
        this.recordOutcomeMetrics(lastOutcome,tool.name);
        this.commandService.recordOutcome?.(lastOutcome.status==="unknown"?"failed":lastOutcome.status);
        previousContext=this.planner.observe(previousContext,userText,plan,step,reply.result);
        if(typeof (this.planner as any).recordVerifiedLearning==="function")(this.planner as any).recordVerifiedLearning(priorContext,userText,plan,step,reply.result,lastOutcome,false);
        if(conversationId)this.runtime?.saveConversationActionContext(conversationId,previousContext);
      }
    ...
    hooks.onReplaceText?.(finalReply.text);hooks.onStatus?.("Tarefa concluída.");this.metrics?.record("agent.tool_calls",done.length,{origin:plan.origin??"unknown"});this.metrics?.record("agent.iterations",steps.length,{origin:plan.origin??"unknown"});if(persistedRun)this.runtime?.finish(persistedRun.id,"COMPLETED",finalReply.text);if(lastOutcome?.status==="partial"){const note=`Resultado parcial: ${lastOutcome.unmetGoals.join("; ")}.`;if(!finalReply.text.includes(note))finalReply={...finalReply,text:`${finalReply.text}\n\n${note}`};}
    return {...finalReply,responseMode:plan.responseMode,engine:plan.origin==="fast"?"fast-path":"legacy",toolsUsed:done.map(item=>item.step.tool),...(lastOutcome?{outcome:lastOutcome}:{})};
  }
  ...
  async resumeApproval(checkpointId:string,hooks:AgentRunHooks={}):Promise<AgentReply>{
    ...
    while(state.nextStep<state.steps.length){this.assertNotAborted(hooks.signal);if(state.iteration>=AGENT_LIMITS.maxIterations||state.results.length>=AGENT_LIMITS.maxToolCalls){const text="O fluxo retomado atingiu o limite seguro de etapas.";this.runtime.finish(resumed.run.id,"FAILED",text);return{text,results:state.results};}
      ...
      if(reply.result){hooks.onToolResult?.(step.tool,data,reply.result);const plan:Plan={intent:state.intent,responseMode:state.responseMode};const before=previous;const outcome=await this.verifyOutcome(state.userRequest,step.tool,reply.result,hooks.signal);this.recordOutcomeMetrics(outcome,step.tool);this.commandService.recordOutcome?.(outcome.status==="unknown"?"failed":outcome.status);previous=this.planner.observe(previous,state.userRequest,plan,step,reply.result);if(typeof (this.planner as any).recordVerifiedLearning==="function")(this.planner as any).recordVerifiedLearning(before,state.userRequest,plan,step,reply.result,outcome,true);if(hooks.visualContext?.conversationId)this.runtime.saveConversationActionContext(hooks.visualContext.conversationId,previous);}
  ...
  private async preflightStep(step:PlanStep,hooks:AgentRunHooks):Promise<{tool:ToolDefinition;data:Record<string,unknown>;action:PreparedAction}|{reply:AgentReply}>{await this.ensureToolCapability?.(step.tool);const result=await this.actionExecutor.preflight(step.tool,step.input,{...this.executionContext(hooks),executionId:step.executionId,...(!this.connections?{capabilityResolver:()=>true}:{})});if(!result.ok){if(result.code==="PATH_DENIED"||result.code==="CAPABILITY_DENIED"||result.code==="SECURITY_DENIED")hooks.onStatus?.("A execução foi bloqueada pelas permissões locais.");return{reply:{text:`Não consegui concluir a ação: ${result.message}`}};}const tool=this.registry.get(result.action.toolName);if(!tool)return{reply:{text:`Ferramenta não disponível: ${result.action.toolName}`}};return{tool,data:result.action.input,action:result.action};}
  private async verifyOutcome(userRequest:string,toolName:string,result:ToolResult,signal?:AbortSignal):Promise<GoalOutcome>{
    if(this.accuracy?.outcomeEnabled?.()===true)return(this.accuracy.outcomeVerifier??new OutcomeVerifier()).verifyWithSemanticFallback({userRequest,toolName,result},signal);
    return result.ok?{status:"unknown",verified:false,reason:"OUTCOME_VERIFIER_DISABLED"}:{status:"failed",verified:true,reason:result.error?.message??result.summary};
  }
  private recordOutcomeMetrics(outcome:GoalOutcome,toolName:string){if(outcome.status==="partial")this.metrics?.record("intent.goal.partial",1,{tool:toolName});else if(outcome.status==="failed")this.metrics?.record("intent.goal.not_satisfied",1,{tool:toolName});else if(outcome.status==="success")this.metrics?.record("intent.goal.verified",1,{tool:toolName});}
...
function userFacingFilesystemError(toolName:string,code:string,detail:string){if(!/^(?:find_file|search_files|list_files|largest_files|read_file|file_info|create_folder|create_text_file|write_text_file|copy_file|move_file|rename_file|open_path)$/.test(toolName))return detail;if(/FILE_ALREADY_EXISTS|FILE_EXISTS/.test(detail))return"Esse arquivo já existe; não foi sobrescrito.";if(/FILE_NOT_FOUND/.test(detail))return"O arquivo informado não foi encontrado.";if(code==="PATH_DENIED"||/PATH_DENIED|fora do escopo permitido/i.test(detail))return"Esse caminho não está autorizado. Revise Configurações → Segurança → Pastas permitidas.";if(/ENOENT|no such file or directory/i.test(detail))return"Não foi possível acessar o destino. Confirme se a pasta autorizada ainda existe.";if(/EACCES|EPERM|permission denied/i.test(detail))return"O sistema negou acesso ao caminho. Verifique a permissão da pasta autorizada.";if(/FILESYSTEM_SEARCH_LIMIT/.test(detail))return"A busca atingiu o limite de segurança antes de concluir.";if(/FILESYSTEM_SEARCH_CANCELLED|AbortError/i.test(detail))return"A busca foi cancelada.";if(["create_text_file","create_folder","write_text_file"].includes(toolName))return"Não foi possível criar ou atualizar o arquivo. Verifique o destino e as permissões da pasta.";return"Não foi possível concluir a operação de arquivo. Verifique se o caminho ainda está acessível e autorizado.";}
