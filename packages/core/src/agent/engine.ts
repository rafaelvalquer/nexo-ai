...
function commandRoutePlan(route:Extract<CommandRoute,{type:"tool"|"macro"}>):Plan{
  if(route.type==="macro")return{tool:`macro_${route.operation}`,input:route.input,explanation:route.explanation,origin:"fast"};
  const step:PlanStep={tool:route.tool,input:route.input,explanation:route.explanation,approval:route.approval,executionId:route.executionId};
  return{tool:route.tool,input:route.input,steps:[step],explanation:route.explanation,origin:"fast",responseMode:route.responseMode,intent:route.intent};
}
