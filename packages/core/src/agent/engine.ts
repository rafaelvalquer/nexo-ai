...
export class AgentEngine{
  ...
  private async executeSteps(userText:string,steps:PlanStep[],plan:Pick<Plan,"origin"|"intent"|"deferredAction"|"responseMode">,hooks:AgentRunHooks,context:LLMMessage[]):Promise<AgentReply>{
    ...
    for(let stepIndex=0;stepIndex<steps.length;stepIndex++){
      ...
      if(reply.result){
        ...
        lastOutcome=this.verifyOutcome(userText,tool.name,reply.result);
  ...
  private verifyOutcome(userRequest:string,toolName:string,result:ToolResult):GoalOutcome{
    if(this.accuracy?.outcomeEnabled?.()===true)return(this.accuracy.outcomeVerifier??new OutcomeVerifier()).verify({userRequest,toolName,result});
