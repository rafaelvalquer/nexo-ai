...
export class AgentEngine{
  ...
  async run(userText:string,hooks:AgentRunHooks={},context:LLMMessage[]=[]):Promise<AgentReply>{
    ...
    const shadowStarted=Date.now(),shadow=this.agentLoopMode()==="shadow"?new ShadowAgent(this.planner.agentProvider(),this.toolCatalog).decide(userText,context.map(message=>({...message,trust:"TRUSTED_LOCAL" as const})),hooks.signal).catch(()=>({kind:"invalid" as const})):undefined;
    ...
    if(this.agentLoopMode()==="read_only")return this.runAgentLoop(userText,hooks,context,"read_only");
    if(this.agentLoopMode()==="full"){try{return await this.runAgentLoop(userText,hooks,context,"full");}catch(error){this.metrics?.record("agent.legacy_fallback",1,{reason:error instanceof Error?error.name:"unknown"});hooks.onStatus?.("Agent V2 indisponível; usando o modo de compatibilidade seguro.");}}
    let plan:Plan;
    try{hooks.onStatus?.("Interpretando sua intenção com a IA local…");plan=await this.planner.plan(userText,context,hooks.signal,previous,this.toolCatalog.list());if(plan.origin==="fast")this.metrics?.record("agent.fast_path_hit",1);if(shadow)this.recordShadow(await shadow,plan,Date.now()-shadowStarted);}
  ...
  private async executePlan(userText: string, plan: Plan, hooks: AgentRunHooks, context: LLMMessage[]): Promise<AgentReply> {
    ...
    for(let stepIndex=0;stepIndex<steps.length;stepIndex++){
      ...
      const reply=await this.execute(tool.name,data,{...this.executionContext(hooks),executionId:step.executionId});hooks.onToolCompleted?.(tool.name,Boolean(reply.result?.ok));if(persistedRun)this.runtime?.recordStep(persistedRun.id,stepIndex,step,reply.result?.ok?"COMPLETED":"FAILED",reply.result,reply.result?.error);
  ...
  async resumeApproval(checkpointId:string,hooks:AgentRunHooks={}):Promise<AgentReply>{
    ...
    while(state.nextStep<state.steps.length){this.assertNotAborted(hooks.signal);if(state.iteration>=AGENT_LIMITS.maxIterations||state.results.length>=AGENT_LIMITS.maxToolCalls){const text="O fluxo retomado atingiu o limite seguro de etapas.";this.runtime.finish(resumed.run.id,"FAILED",text);return{text,results:state.results};}
      ...
      this.runtime.recordStep(resumed.run.id,state.nextStep,step,"RUNNING");hooks.onStatus?.(step.explanation??`Retomando ${step.tool}…`);hooks.onToolStarted?.(step.tool,step.explanation??`Retomando ${step.tool}`);const approved=this.approvals.list("all").find(item=>item.checkpointId===checkpointId&&item.status==="approved");const reply=await this.execute(step.tool,data,{...this.executionContext(hooks),executionId:step.executionId,approvalId:approved?.id});this.assertNotAborted(hooks.signal);hooks.onToolCompleted?.(step.tool,Boolean(reply.result?.ok));this.runtime.recordStep(resumed.run.id,state.nextStep,step,reply.result?.ok?"COMPLETED":"FAILED",reply.result,reply.result?.error);if(!reply.result?.ok){this.runtime.finish(resumed.run.id,"FAILED",reply.text);return{...reply,results:state.results};}
  ...
  private recordShadow(shadow:ShadowDecision,plan:Plan,latencyMs:number){const legacy=plan.steps?.[0]?.tool??plan.tool??(plan.direct||plan.directStream?"final":"none"),v2=shadow.kind==="tool"?shadow.toolName:shadow.kind,agreement=legacy===v2,finalIntent=plan.intent?.operation??plan.intent?.intent??"unknown";this.metrics?.record(agreement?"agent.shadow.match":"agent.shadow.mismatch",1,{legacy_plan:legacy,v2_first_tool:v2,v2_final_intent:finalIntent,agreement});this.metrics?.record("agent.shadow.latency_ms",latencyMs,{v2_first_tool:v2});this.metrics?.record("agent.shadow.tool_selection_accuracy",agreement?1:0,{legacy,v2});if(shadow.kind==="tool"&&shadow.mutatesState)this.metrics?.record("agent.shadow.unexpected_mutation",1,{tool:shadow.toolName});}
