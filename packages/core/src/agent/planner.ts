...
export class AgentPlanner{
  ...
  async plan(userText:string,context:LLMMessage[]=[],signal?:AbortSignal,previous?:ConversationActionContextState,availableTools?:AgentToolDescriptor[]):Promise<Plan>{
    ...
    const hint=resolveDomainHint(userText),store=this.activeIntentMemory(),retriever=this.retrieverFor(store),learned=retriever&&this.isIntentLearningEnabled()?await retriever.retrieve(userText,hint?.domain,5).catch(()=>[]):[];const interpreted=await this.orchestrator.interpret(userText,tools,{previous,learnedExamples:learned},context,signal),intent=enrichFilesystemIntent(interpreted,userText),built=buildIntentPlan(intent,tools,previous);return{...built,steps:built.steps as PlanStep[]|undefined,origin:"llm",intent};
  }
  buildIntentPlan(intent:AgentIntent,previous?:ConversationActionContextState,availableTools?:AgentToolDescriptor[]):Plan{const tools=availableTools??this.toolDescriptors();const built=buildIntentPlan(intent,tools,previous);return{...built,steps:built.steps as PlanStep[]|undefined,origin:"fast",intent};}
