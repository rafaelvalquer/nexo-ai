...
export class EntityReferenceResolver{
  ...
  resolve(conversationId:string,text:string){
    ...
    const isCurrentPage=/\b(esta|essa|atual)\s+(pagina|site|artigo)\b/.test(normalized);
    const isLast=/\b([uú]ltimo|[uú]ltima)\b/.test(normalized)||isCurrentPage;
    ...
    const kind:ConversationEntity["kind"]|undefined=isCurrentPage?"page":/e-?mail|mensagem/.test(normalized)?"email":/evento|compromisso|reuniao/.test(normalized)?"event":/documento/.test(normalized)?"document":/arquivo|pasta|path|caminho/.test(normalized)?"file":undefined;
