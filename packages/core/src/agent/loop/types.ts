...
export type AgentLoopStatus = "DECIDING" | "PREFLIGHT" | "WAITING_APPROVAL" | "EXECUTING" | "OBSERVING" | "RECONCILING" | "WAITING_EXTERNAL_PROGRESS" | "RESULT_UNKNOWN" | "COMPLETED" | "FAILED" | "CANCELLED";
export type AgentMessageTrust = "TRUSTED_LOCAL" | "UNTRUSTED_CONTENT" | "UNTRUSTED_EXTERNAL" | "SENSITIVE_LOCAL";
export type AgentModelMessage = { role: "system" | "user" | "assistant" | "tool"; content: string; trust?: AgentMessageTrust; toolCallId?: string };
export type AgentToolSchema = { name: string; description: string; parameters?: unknown; polling?: {allowed:true;minIntervalMs:number;maxDurationMs:number;maxAttempts:number;progressFields?:string[]} };
...
export interface AgentLoopState { version: 2; runId: string; conversationId?: string; taskId?: string; userRequest: string; messages: AgentModelMessage[]; observations: AgentObservation[]; pendingAction?: PendingAgentAction; iteration: number; toolCallCount: number; consecutiveFailures: number; protocolRepairCount: number; actionFingerprints: string[]; observationFingerprints: string[]; startedAt: string; updatedAt: string; deadlineAt: string; status: AgentLoopStatus; finalResponse?: string; }

export type AgentLoopLimits = { maxIterations: number; maxToolCalls: number; maxExecutionMs: number; maxProtocolRepairAttempts: number; maxSchemaRepairAttempts: number; maxConsecutiveFailures: number; maxSameActionRepeats: number; maxSameObservationRepeats: number; maxObservationBytes: number; maxTotalObservationBytes: number; maxToolResultReferences: number; };
export const DEFAULT_AGENT_LOOP_LIMITS: AgentLoopLimits = { maxIterations: 10, maxToolCalls: 15, maxExecutionMs: 120_000, maxProtocolRepairAttempts: 2, maxSchemaRepairAttempts: 2, maxConsecutiveFailures: 3, maxSameActionRepeats: 2, maxSameObservationRepeats: 2, maxObservationBytes: 64_000, maxTotalObservationBytes: 256_000, maxToolResultReferences: 100 };
export interface AgentLoopPersistence { save(state: AgentLoopState): Promise<void>; }
export type AgentLoopDependencies = { agentTurn(request: AgentTurnRequest, signal?: AbortSignal): Promise<AgentModelTurn>; tools(): AgentToolSchema[]; preflight(name: string, input: Record<string, unknown>, context: { runId: string; signal?: AbortSignal }): Promise<import("../execution/types.js").ActionPreflightResult>; execute(action: PreparedAction, context: { runId: string; signal?: AbortSignal }): Promise<import("../execution/types.js").ActionExecutionResult>; observe(result: import("../execution/types.js").ActionExecutionResult, callId: string): AgentObservation; persistence?: AgentLoopPersistence; metric?(name:string,value?:number,labels?:Record<string,string|number|boolean>):void; };
