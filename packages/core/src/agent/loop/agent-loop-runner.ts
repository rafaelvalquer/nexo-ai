...
export class AgentLoopRunner {
  ...
  async run(userRequest: string, options: { mode: "read_only" | "full"; runId?: string; conversationId?:string;taskId?:string;messages?:AgentLoopState["messages"];resources?:AgentResourceContext;signal?: AbortSignal } ): Promise<AgentLoopState> {
    ...
    if(currentPage?.kind==="page"&&/\b(leia|ler|resuma|resumir|analise|analisar|extraia|extrair|conte[uú]do)\b/i.test(userRequest)){
      ...
      if(fastPage){const completed=await this.executeFastPath(userRequest,fastPage.name,fastPage.arguments,{runId,conversationId:options.conversationId,taskId:options.taskId,messages,signal:options.signal});if(completed)return completed;}
    }

    const fast=new V2FastPathRouter(locations).resolve(userRequest,available,this.executor.allowedFilesystemRoots());
    if(fast){
  ...
  private createLoop(mode:"read_only"|"full",available=this.availableForMode(mode)){return new AgentLoop({
      ...
      onObservation:(state,observation)=>{if(state.conversationId)this.runtime?.entities.record(state.conversationId,observation,Math.max(1,state.observations.length));},
  ...
  private toObservation(execution:Awaited<ReturnType<ActionExecutor["executePrepared"]>>,callId:string){
    ...
    const observation=encodeObservation(callId,execution.action.toolName,execution.result??{success:false,ok:false,summary:"A execução não retornou resultado.",error:{code:"ACTION_FAILED",message:execution.error??"A execução não retornou resultado."}},trust);
