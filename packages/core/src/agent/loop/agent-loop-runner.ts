...
export class AgentLoopRunner {
  ...
  async resumeApproved(state:AgentLoopState,approvalId:string,coordinator:ApprovalCoordinator,signal?:AbortSignal){if(state.status!=="WAITING_APPROVAL"||!state.pendingAction)throw new Error("Run não está aguardando aprovação.");const pending=state.pendingAction;if(pending.approvalId&&pending.approvalId!==approvalId)throw new Error("Approval não pertence à PendingAgentAction.");const available=this.catalog.list();const schemas=createAgentToolSchemas(available),allowed=new Set(schemas.map(tool=>tool.name));if(!allowed.has(pending.toolName))throw new Error("A ferramenta aprovada não está mais disponível.");const definition=this.registry.get(pending.toolName);if(!definition)throw new Error("A ferramenta aprovada não está registrada.");const action={executionId:pending.executionId,toolName:pending.toolName,input:pending.input,fingerprint:pending.fingerprint,idempotencyKey:pending.idempotencyKey,mutatesState:pending.mutatesState,risk:pending.risk??definition.risk,requiresApproval:pending.requiresApproval??true,status:"PREPARED" as const};coordinator.consumeApproved(approvalId,pending);state.status="EXECUTING";state.executionSafetyState="DISPATCHING";state.updatedAt=new Date().toISOString();await this.runtime?.saveLoopState(state.runId,state);const execution=await this.executor.executePrepared(action,{runId:state.runId,executionId:pending.executionId,dispatchAuthorized:true,signal});return this.createLoop(schemas,allowed).acceptExecution(state,action,execution,signal);}
