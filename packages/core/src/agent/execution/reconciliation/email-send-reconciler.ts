...
export class EmailSendReconciler implements MutationReconciler {
  ...
  async reconcile(record: ExecutionRecord, signal?: AbortSignal): Promise<ReconciliationResult> {
    ...
    try {
      ...
      if (matches.length === 1) return { status: "confirmed_success", result: { ok: true, summary: "Envio confirmado por destinatários, assunto, corpo e janela temporal.", data: { messageId: matches[0].id,threadId:matches[0].threadId } } };
