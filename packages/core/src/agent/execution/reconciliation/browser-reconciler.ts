...
export class BrowserReconciler implements MutationReconciler {
  ...
  async reconcile(record: ExecutionRecord, signal?: AbortSignal): Promise<ReconciliationResult> {
    ...
    if (record.toolName === "browser_screenshot" && typeof record.input.path === "string") {
      try { const stat = await fs.stat(record.input.path); return stat.size > 0 ? { status: "confirmed_success", result: { success:true, ok: true, summary: "Captura confirmada no filesystem." } } : { status: "confirmed_failure" }; } catch { return { status: "confirmed_failure" }; }
    }
    if (record.toolName === "browser_close") {
      const pages = await this.sessions.pages(record.runId ?? "default").catch(() => []);
      return pages.length === 0 ? { status: "confirmed_success", result: { success:true, ok: true, summary: "Sessão do navegador confirmada como fechada." } } : { status: "confirmed_failure" };
    }
