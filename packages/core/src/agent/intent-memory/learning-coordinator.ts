...
export class IntentLearningCoordinator{
 ...
 recordVerified(input:{utterance:string;intent:AgentIntent;source:IntentExampleSource;outcome:GoalOutcome;unresolvedAmbiguity?:boolean;resolverVersion?:string;modelId?:string;embedding?:number[]}){
  ...
  const event=this.events.record({utterancePattern:pattern,domain:input.intent.domain,operation:input.intent.operation,entitiesSignature:this.sanitizer.entitiesSignature(input.intent.entities??{}),source:input.source,outcome,confidence:input.intent.confidence,resolverVersion:input.resolverVersion??"accuracy-context-learning-v1",modelId:input.modelId,failureType:input.outcome.status==="success"?undefined:"goal_not_satisfied"});
 ...
 recordFailure(input:{utterance:string;domain:string;operation:string;confidence:number;failureType:LearningFailureType;resolverVersion?:string;modelId?:string}){
  this.events.record({utterancePattern:this.sanitizer.sanitize(input.utterance),domain:input.domain,operation:input.operation,entitiesSignature:"",source:"negative",outcome:"failed",confidence:input.confidence,resolverVersion:input.resolverVersion??"accuracy-context-learning-v1",modelId:input.modelId,failureType:input.failureType});
