...
function ResourceEntry({item,reference,selected,onSelect}:{item:ResourceItem;reference?:Reference;selected:boolean;onSelect:(value:boolean)=>void}) {
  ...
  const children=<>{reference&&<ResourceActions item={item} onAction={action}/>}
    ...
    {preview&&<div className="resourceDetails" onKeyDown={event=>{if(event.key==="Escape")setPreview(undefined);}}>{preview.kind==="text"?<pre>{preview.content}</pre>:preview.kind==="image"?<img src={preview.content} alt={resource.kind==="file"?resource.name:"Prévia do arquivo"}/>:<iframe title="Prévia do PDF" src={preview.content}/>}<button type="button" onClick={()=>setPreview(undefined)}>Fechar prévia</button>{resource.kind==="file"&&item.actions.some(action=>action.id==="file.open")&&<button type="button" onClick={()=>void run("file.open").catch(()=>{})}>Abrir externamente</button>}</div>}
  ...
  return resource.kind==="generic"?<GenericResourceCard item={item} resource={resource}>{children}</GenericResourceCard>:null;
}
