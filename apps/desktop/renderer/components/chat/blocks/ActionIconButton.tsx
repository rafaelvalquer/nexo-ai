...
export function ActionIconButton({action,busy,onClick}:{action:ResourceAction;busy?:boolean;onClick:()=>void}) {
  ...
  return <Tooltip content={action.label}><button type="button" className={`resourceActionButton ${action.icon==="trash"?"danger":""} ${busy?"loading":""}`} aria-label={action.label} disabled={action.disabled||busy} onClick={onClick}><Icon size={16}/></button></Tooltip>;
}
