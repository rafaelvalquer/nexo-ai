...
export function EmailComposeReviewBlock({block}:{block:EmailComposeReviewModel}){
  ...
  const[draft,setDraft]=useState<EmailComposeDraftSnapshot>(initial),[fields,setFields]=useState<Fields>({...block.fields,to:[...block.fields.to]}),[busy,setBusy]=useState(false),[error,setError]=useState(block.error??"");
  ...
  useEffect(()=>{
    let active=true;
    void (async()=>{try{const current=await window.nexo.getEmailDraft(block.draftId) as EmailComposeDraftSnapshot;if(active){applyDraft(current);saveChain.current=Promise.resolve(current);}}catch(caught){if(active)setError(caught instanceof Error?caught.message:String(caught));}})();
    requestAnimationFrame(()=>requestAnimationFrame(()=>rootRef.current?.scrollIntoView({behavior:"smooth",block:"center"})));
    return()=>{active=false;};
  },[applyDraft,block.draftId]);
  ...
  useEffect(()=>{
    ...
    timerRef.current=window.setTimeout(()=>{void queueSave(snapshot).catch(caught=>setError(caught instanceof Error?caught.message:String(caught)));},400);
    return()=>{if(timerRef.current!==undefined)window.clearTimeout(timerRef.current);};
  },[busy,draft.status,fields,queueSave]);
