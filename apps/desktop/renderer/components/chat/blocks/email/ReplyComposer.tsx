...
export function ReplyComposer({recipient,onCancel,onSubmit}:{recipient:string;onCancel:()=>void;onSubmit:(body:string)=>Promise<void>}) {
  ...
  async function submit(){if(!body.trim()||busy)return;setBusy(true);setError("");try{await onSubmit(body);}catch(error){setError(error instanceof Error?error.message:String(error));}finally{setBusy(false);}}
