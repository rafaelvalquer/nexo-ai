...
export function ActionForm({actionId,item,onCancel,onSubmit}:{actionId:string;item:ResourceItem;onCancel:()=>void;onSubmit:(values:NonNullable<ChatActionRequest["values"]>)=>Promise<void>}) {
  ...
  return <form className="resourceActionForm" onKeyDown={event=>{if(event.key==="Escape")onCancel();}} onSubmit={async event=>{event.preventDefault();setBusy(true);setError("");try{const submitted={...values} as NonNullable<ChatActionRequest["values"]>;if(actionId==="calendar.edit"){submitted.start=new Date(values.start).toISOString();submitted.end=new Date(values.end).toISOString();}await onSubmit(submitted);}catch(error){setError(error instanceof Error?error.message:String(error));}finally{setBusy(false);}}}>
    ...
    {actionId==="calendar.rsvp"&&<label>Resposta ao convite<select autoFocus value={values.response} onChange={event=>setValues({response:event.target.value})}><option value="accept">Aceitar</option><option value="tentative">Talvez</option><option value="decline">Recusar</option></select></label>}
