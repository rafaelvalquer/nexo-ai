...
export function BrowserApproval({event}:{event:ApprovalEvent}){
  ...
  async function decide(approved:boolean){if(busy||resolved)return;setBusy(true);setError("");try{await resolve(event.approvalId,approved);setResolved(approved?"approved":"rejected");}catch(error){setError(userFacingError(error,"Não consegui registrar sua decisão. Tente novamente.",diagnostics));}finally{setBusy(false);}}
