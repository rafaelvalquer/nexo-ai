...
export function BrowserTimeline({events,history}:{events:BrowserRunEvent[];history:StoredEvent[]}){
  ...
  const rows=[...new Map([...historical,...live].map(item=>[item.key,item])).values()].sort((a,b)=>Date.parse(a.time)-Date.parse(b.time)).slice(-12);
...
function labelForLive(event:BrowserRunEvent){if(event.type==="browser.step")return event.label;if(event.type==="browser.navigation")return`Navegou para ${safeHost(event.url)}`;if(event.type==="browser.approval_requested")return event.label;if(event.type==="browser.status")return event.status==="paused"?"Execução pausada":event.status==="running"?"Navegador ativo":event.status==="waiting_approval"?"Aguardando aprovação":"Estado atualizado";if(event.type==="browser.completed")return"Execução concluída";if(event.type==="browser.failed")return event.error;if(event.type==="browser.cancelled")return"Execução cancelada";return"Navegador iniciado";}
