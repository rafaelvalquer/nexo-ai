...
export function ClarificationOptionButton({option,selected,disabled,onSelect}:{option:ClarificationOption;selected:boolean;disabled:boolean;onSelect:(option:ClarificationOption)=>void}){
  return <button type="button" className={`structuredClarificationOption${selected?" selected":""}`} disabled={disabled} aria-pressed={selected} aria-selected={selected} role="option" onClick={()=>onSelect(option)}>
