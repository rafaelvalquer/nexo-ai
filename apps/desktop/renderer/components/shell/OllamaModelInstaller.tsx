...
export function OllamaModelInstaller() {
  ...
  return <aside className="modelInstaller" role="status"><b>Modelo local necessário</b><span>{busy ? `${progress?.status ?? "Baixando…"}${percent === undefined ? "" : ` · ${percent}%`}` : `${model} ainda não está instalado.`}</span>{error&&<span className="notice error" role="alert">{error}</span>}<button disabled={busy} onClick={()=>void pull()}>{busy ? "Baixando…" : error?"Tentar novamente":"Baixar modelo"}</button></aside>;
}
