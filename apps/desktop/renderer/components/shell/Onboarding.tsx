...
export function Onboarding() {
  ...
  const openSettings=async()=>{await complete();setPage("Configurações");};
  return <NexoDialog open title="Seu assistente local está pronto para ser configurado." eyebrow="BEM-VINDO AO NEXO" onClose={()=>void complete()} closeLabel="Pular configuração" className="onboardingCard"><ol><li className={ollama?.ok?"done":""}><b>IA local</b><span>{ollama?.ok?"Ollama está disponível.":ollama?"Ollama não encontrado. Instale/inicie o serviço e volte a testar em Configurações.":"Verificando Ollama…"}</span></li><li><b>Contas</b><span>Conecte Google ou Microsoft para e-mail e calendário, quando quiser.</span><button className="ghost" onClick={()=>void openSettings()}>Abrir Configurações</button></li><li><b>Autonomia e pastas</b><span>Defina permissões e pastas autorizadas antes de automatizar tarefas.</span><button className="ghost" onClick={()=>void openSettings()}>Abrir Configurações</button></li></ol><div className="actions"><button className="ghost" onClick={()=>void complete()}>Pular por enquanto</button><button onClick={()=>void complete()}>Concluir</button></div></NexoDialog>;
}
