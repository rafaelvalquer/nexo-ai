...
export const useAssistantStore=create<Store>((set,get)=>({blockLimits:{},
  ...
  attachDroppedFiles:async(sessionId,files)=>{const supported=files.filter(file=>/\.(pdf|docx|txt|md)$/i.test(file.name)),rejected=files.filter(file=>!supported.includes(file));if(rejected.length)set({error:`Formato não suportado para ${rejected.map(file=>file.name).join(", ")}. Use PDF, DOCX, TXT ou MD.`});for(const file of supported){try{const task=await window.nexo.importDroppedDocument(file) as BackgroundTask|undefined;if(task)set(state=>({sessions:state.sessions.map(session=>session.id===sessionId?{...session,attachments:[...session.attachments,{taskId:task.id,name:file.name,status:"indexing"}]}:session)}));}catch(error){set({error:`Não foi possível anexar ${file.name}: ${error instanceof Error?error.message:"tente novamente."}`});}}},
