...
export function useConnections(){
  const[accounts,setAccounts]=useState<ConnectionAccount[]>([]),[configuration,setConfiguration]=useState<OAuthConfiguration>(empty),[loading,setLoading]=useState(true),[loaded,setLoaded]=useState(false),[loadError,setLoadError]=useState(false),[busy,setBusy]=useState<ConnectionProvider|"settings"|"secret"|null>(null),[error,setError]=useState<string|null>(null),[notice,setNotice]=useState<string|null>(null),loadedRef=useRef(false);
  const reload=useCallback(async()=>{if(!loadedRef.current)setLoading(true);try{const[next,config]=await Promise.all([window.nexo.listConnections(),window.nexo.getConnectionConfiguration()]);setAccounts(next);setConfiguration(config);loadedRef.current=true;setLoaded(true);setLoadError(false);}catch(cause){console.error("Falha ao carregar integrações locais",cause);setLoadError(true);}finally{setLoading(false);}},[]);
  useEffect(()=>{void reload();},[reload]);
  const run=useCallback(async<T,>(key:ConnectionProvider|"settings"|"secret",task:()=>Promise<T>)=>{setBusy(key);setError(null);setNotice(null);try{return await task();}catch(cause){console.error("Falha em operação de integração",cause);setError("A operação não foi concluída. Verifique a conexão e tente novamente.");throw cause;}finally{setBusy(null);}},[]);
  return {accounts,configuration,setConfiguration,loading,loaded,loadError,busy,error,notice,setError,setNotice,reload,
