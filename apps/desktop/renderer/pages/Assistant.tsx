...
export function Assistant(){
  ...
  const scroll=useChatAutoScroll(viewportRef,`${assistant.messages.length}:${assistant.activeTask?.progressText?.length??0}`,assistant.isStreaming);
  const send=async(value:string)=>{const sent=await assistant.send(value);if(sent)requestAnimationFrame(()=>scroll.scrollToBottom("smooth"));return sent;};
  return <AssistantShell>
    <AssistantHeader model="qwen3:4b" busy={assistant.isStreaming} elapsed={assistant.elapsed} onExecution={()=>setDrawerOpen(true)}/>
    ...
    <ScrollToBottom visible={!scroll.isNearBottom} unread={scroll.hasUnreadBelow} onClick={()=>scroll.scrollToBottom("smooth")}/>
