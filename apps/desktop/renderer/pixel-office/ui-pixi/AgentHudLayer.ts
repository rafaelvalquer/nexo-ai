...
export class AgentHudLayer extends Container{
  ...
  updateAgents(agents:Iterable<OfficeAgent>,width:number,height:number,delta:number){
    ...
    const anchors=list.map(agent=>{
      ...
      return{id:agent.id,x:point.x,y:point.y,width:bubble.bubbleWidth,height:bubble.bubbleHeight};
    });

    for(const placement of this.layoutManager.layout(anchors,width,height)){
      const bubble=this.bubbles.get(placement.id);
      if(bubble)bubble.position.set(placement.x,placement.y);
    }
