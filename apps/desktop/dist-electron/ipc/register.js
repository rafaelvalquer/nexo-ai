...
export function registerIpc(core, desktop) {
    ...
    ipcMain.handle("nexo:memory:list", () => core.memory.list());
    ipcMain.handle("nexo:memory:add", (_, content) => core.addMemory(content));
    ipcMain.handle("nexo:memory:remove", (_, id) => core.memory.remove(id));
