...
test("Agent V2 resolves Downloads, creates missing parents, and overrides the model destination", async () => {
  ...
  try {
    ...
    expect(fs.readFileSync(expected, "utf8")).toBe("Olá Nexo");
...
test("Agent V2 auto-corrects downlaod before approval", async () => {
  test.setTimeout(120_000);
  ...
  try {
    ...
    await page.evaluate(id => window.nexo.resolveApproval(id, true), approval!.id);
    await expect.poll(async () => page.evaluate(id => window.nexo.getTask(id).then(item => item?.status), task.id), { timeout: 30_000 }).toBe("completed");
    expect(fs.readFileSync(expected, "utf8")).toBe("fuzzy");
  } finally {
    await app.close();
    await new Promise<void>(resolve => ollama.close(() => resolve()));
    fs.rmSync(dataDir, { recursive: true, force: true });
