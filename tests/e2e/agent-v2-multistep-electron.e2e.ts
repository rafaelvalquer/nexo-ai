...
test("Agent V2 full executes a stateful single-tool sequence and creates exactly one approved file",async()=>{
  ...
  try{
    ...
    await page.evaluate(root=>window.nexo.updateSettings({allowedRoots:[root],fileWritesEnabled:true,agentLoopMode:"full",agentLegacyFallbackEnabled:false}),files);
    ...
    expect(calls.filter(call=>call.tools.length)).toHaveLength(2);
...
test("E2E-06 fileWritesEnabled changed while waiting approval blocks dispatch",async()=>{
  ...
  try{const task=await start(harness,"Crie policy.txt na pasta permitida",[]);await waitApproval(harness,task.id);await harness.page.evaluate(()=>window.nexo.updateSettings({fileWritesEnabled:false}));await expect(harness.page.evaluate(()=>window.nexo.listApprovals().then(rows=>window.nexo.resolveApproval((rows.find((row:any)=>row.status==="pending") as any).id,true)))).rejects.toThrow(/TOOL_DISABLED/);expect(fs.existsSync(path.join(harness.files,"policy.txt"))).toBe(false);}
  finally{await harness.close();}
});
...
test("E2E-07 crash at dispatch recovers without a second write",async()=>{
  ...
  try{let page=await app.firstWindow();await expect(page.locator("#root .app")).toBeVisible();await page.evaluate(root=>window.nexo.updateSettings({allowedRoots:[root],fileWritesEnabled:true,agentLoopMode:"full",agentLegacyFallbackEnabled:false}),files);const conversation=await page.evaluate(()=>window.nexo.createConversation("crash")),task=await page.evaluate(({id,target})=>window.nexo.startChatTask(id,`Crie ${target}`,[]),{id:conversation.id,target});await expect.poll(()=>page.evaluate(id=>window.nexo.getTask(id).then(task=>task?.status),task.id),{timeout:30_000}).toBe("waiting_approval");const approval=await page.evaluate(()=>window.nexo.listApprovals().then(rows=>rows.find((row:any)=>row.status==="pending")));const crashed=app.process();const exited=new Promise<void>(resolve=>crashed.once("exit",()=>resolve()));void page.evaluate(id=>window.nexo.resolveApproval(id,true),approval!.id).catch(()=>undefined);await exited;
    app=await launch();page=await app.firstWindow();await expect(page.locator("#root .app")).toBeVisible();await expect.poll(()=>page.evaluate(()=>window.nexo.status().then((status:any)=>status.agentDiagnostics.reconciliationStatus)),{timeout:30_000}).toBe("RECONCILED_SUCCESS");expect(fs.readFileSync(target,"utf8")).toBe(content);expect(fs.readdirSync(files).filter(name=>name==="crash.txt")).toHaveLength(1);
