...
afterEach(()=>{globalThis.fetch=originalFetch;vi.restoreAllMocks();tempDirs.splice(0).forEach(dir=>fs.rmSync(dir,{recursive:true,force:true}));});
...
describe("Dashboard email trash",()=>{
  it("routes dashboardEmailTrash through email_trash in AgentEngine",async()=>{
    ...
    const core=new NexoCore({dataDir:dir});await core.ready();core.updateSettings({connectionsEnabled:true});await core.ensureConnections();
