...
describe("verify_goal",()=>{
 it("routes partial goal back for correction",()=>{const result=verifyGoalNode(state(0));expect(result.loopState?.goalOutcome?.status).toBe("partial");expect(result.loopState?.status).toBe("DECIDING");expect(result.loopState?.correctionRounds).toBe(1);});
 it("stops after two correction rounds",()=>{const result=verifyGoalNode(state(2));expect(result.loopState?.status).toBe("FAILED");});
});
